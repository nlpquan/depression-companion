...
class TextProcessor(BaseProcessor[str, TextFeatures]):
    ...
    def validate_input(self, input_data: str) -> bool:
        ...
        if len(input_data) > 10000:
            raise ValidationError(
                f"Text too long: {len(input_data)} chars (max 10000)"
            )
    ...
    def _extract_linguistic_features(self, text: str) -> np.ndarray:
        ...
        negation_words = sum(1 for w in words if w in {"not", "no", "never", "neither", "nor", "nothing"})
    ...
    def _extract_keywords(self, text: str) -> np.ndarray:
        ...
        anxiety_keywords = {
            ...
            "stress", "stressed", "overwhelmed", "restless", "tense",
        }
        sleep_keywords = {
            "insomnia", "sleep", "tired", "fatigue", "exhausted",
            "nightmare", "awake", "restless",
        }
